// This file is auto-generated by @hey-api/openapi-ts

import type {Client, Options as Options2, TDataShape} from "./client";
import {client} from "./client.gen";
import type {
  AuthLoginData,
  AuthLoginErrors,
  AuthLoginResponses,
  AuthLogoutData,
  AuthLogoutErrors,
  AuthLogoutResponses,
  AuthMeData,
  AuthMeErrors,
  AuthMeResponses,
  AuthRefreshData,
  AuthRefreshErrors,
  AuthRefreshResponses,
  PermissionsGetAllData,
  PermissionsGetAllErrors,
  PermissionsGetAllResponses,
  RolesAddPermissionData,
  RolesAddPermissionErrors,
  RolesAddPermissionResponses,
  RolesCreateData,
  RolesCreateErrors,
  RolesCreateResponses,
  RolesDeleteData,
  RolesDeleteErrors,
  RolesDeleteResponses,
  RolesGetAllData,
  RolesGetAllErrors,
  RolesGetAllResponses,
  RolesGetByIdData,
  RolesGetByIdErrors,
  RolesGetByIdResponses,
  RolesGetPermissionsData,
  RolesGetPermissionsErrors,
  RolesGetPermissionsResponses,
  RolesRemovePermissionData,
  RolesRemovePermissionErrors,
  RolesRemovePermissionResponses,
  RolesUpdateData,
  RolesUpdateErrors,
  RolesUpdateResponses,
  UsersAddPermissionData,
  UsersAddPermissionErrors,
  UsersAddPermissionResponses,
  UsersAddRoleData,
  UsersAddRoleErrors,
  UsersAddRoleResponses,
  UsersCreateData,
  UsersCreateErrors,
  UsersCreateResponses,
  UsersDeleteData,
  UsersDeleteErrors,
  UsersDeleteResponses,
  UsersGetAllData,
  UsersGetAllErrors,
  UsersGetAllResponses,
  UsersGetByIdData,
  UsersGetByIdErrors,
  UsersGetByIdResponses,
  UsersGetPermissionsData,
  UsersGetPermissionsErrors,
  UsersGetPermissionsResponses,
  UsersGetRolesData,
  UsersGetRolesErrors,
  UsersGetRolesResponses,
  UsersRemovePermissionData,
  UsersRemovePermissionErrors,
  UsersRemovePermissionResponses,
  UsersRemoveRoleData,
  UsersRemoveRoleErrors,
  UsersRemoveRoleResponses,
  UsersUpdateData,
  UsersUpdateErrors,
  UsersUpdateResponses,
  WeatherForecastGetData,
  WeatherForecastGetResponses,
} from "./types.gen";

export type Options<
  TData extends TDataShape = TDataShape,
  ThrowOnError extends boolean = boolean,
  TResponse = unknown,
> = Options2<TData, ThrowOnError, TResponse> & {
  /**
   * You can provide a client instance returned by `createClient()` instead of
   * individual options. This might be also useful if you want to implement a
   * custom client.
   */
  client?: Client;
  /**
   * You can pass arbitrary values through the `meta` object. This can be
   * used to access values that aren't defined as part of the SDK function.
   */
  meta?: Record<string, unknown>;
};

/**
 * Authenticate with username and password.
 */
export const authLogin = <ThrowOnError extends boolean = false>(
  options: Options<AuthLoginData, ThrowOnError>,
) =>
  (options.client ?? client).post<AuthLoginResponses, AuthLoginErrors, ThrowOnError>({
    url: "/api/auth/login",
    ...options,
    headers: {
      "Content-Type": "application/json",
      ...options.headers,
    },
  });

/**
 * Refresh an access token using a valid refresh token.
 */
export const authRefresh = <ThrowOnError extends boolean = false>(
  options: Options<AuthRefreshData, ThrowOnError>,
) =>
  (options.client ?? client).post<AuthRefreshResponses, AuthRefreshErrors, ThrowOnError>({
    url: "/api/auth/refresh",
    ...options,
    headers: {
      "Content-Type": "application/json",
      ...options.headers,
    },
  });

/**
 * Revoke the current refresh token (logout).
 */
export const authLogout = <ThrowOnError extends boolean = false>(
  options: Options<AuthLogoutData, ThrowOnError>,
) =>
  (options.client ?? client).post<AuthLogoutResponses, AuthLogoutErrors, ThrowOnError>({
    url: "/api/auth/logout",
    ...options,
    headers: {
      "Content-Type": "application/json",
      ...options.headers,
    },
  });

/**
 * Get the currently authenticated user's info, roles and permissions.
 */
export const authMe = <ThrowOnError extends boolean = false>(
  options?: Options<AuthMeData, ThrowOnError>,
) =>
  (options?.client ?? client).get<AuthMeResponses, AuthMeErrors, ThrowOnError>({
    url: "/api/auth/me",
    ...options,
  });

/**
 * Get all available static permissions defined in the system.
 */
export const permissionsGetAll = <ThrowOnError extends boolean = false>(
  options?: Options<PermissionsGetAllData, ThrowOnError>,
) =>
  (options?.client ?? client).get<
    PermissionsGetAllResponses,
    PermissionsGetAllErrors,
    ThrowOnError
  >({url: "/api/permissions", ...options});

/**
 * List all roles.
 */
export const rolesGetAll = <ThrowOnError extends boolean = false>(
  options?: Options<RolesGetAllData, ThrowOnError>,
) =>
  (options?.client ?? client).get<RolesGetAllResponses, RolesGetAllErrors, ThrowOnError>({
    url: "/api/roles",
    ...options,
  });

/**
 * Create a new role.
 */
export const rolesCreate = <ThrowOnError extends boolean = false>(
  options: Options<RolesCreateData, ThrowOnError>,
) =>
  (options.client ?? client).post<RolesCreateResponses, RolesCreateErrors, ThrowOnError>({
    url: "/api/roles",
    ...options,
    headers: {
      "Content-Type": "application/json",
      ...options.headers,
    },
  });

/**
 * Delete a role.
 */
export const rolesDelete = <ThrowOnError extends boolean = false>(
  options: Options<RolesDeleteData, ThrowOnError>,
) =>
  (options.client ?? client).delete<RolesDeleteResponses, RolesDeleteErrors, ThrowOnError>({
    url: "/api/roles/{id}",
    ...options,
  });

/**
 * Get a role by ID.
 */
export const rolesGetById = <ThrowOnError extends boolean = false>(
  options: Options<RolesGetByIdData, ThrowOnError>,
) =>
  (options.client ?? client).get<RolesGetByIdResponses, RolesGetByIdErrors, ThrowOnError>({
    url: "/api/roles/{id}",
    ...options,
  });

/**
 * Update a role's name.
 */
export const rolesUpdate = <ThrowOnError extends boolean = false>(
  options: Options<RolesUpdateData, ThrowOnError>,
) =>
  (options.client ?? client).put<RolesUpdateResponses, RolesUpdateErrors, ThrowOnError>({
    url: "/api/roles/{id}",
    ...options,
    headers: {
      "Content-Type": "application/json",
      ...options.headers,
    },
  });

/**
 * Get permissions assigned to a role.
 */
export const rolesGetPermissions = <ThrowOnError extends boolean = false>(
  options: Options<RolesGetPermissionsData, ThrowOnError>,
) =>
  (options.client ?? client).get<
    RolesGetPermissionsResponses,
    RolesGetPermissionsErrors,
    ThrowOnError
  >({url: "/api/roles/{id}/permissions", ...options});

/**
 * Assign a permission to a role.
 */
export const rolesAddPermission = <ThrowOnError extends boolean = false>(
  options: Options<RolesAddPermissionData, ThrowOnError>,
) =>
  (options.client ?? client).post<
    RolesAddPermissionResponses,
    RolesAddPermissionErrors,
    ThrowOnError
  >({
    url: "/api/roles/{id}/permissions",
    ...options,
    headers: {
      "Content-Type": "application/json",
      ...options.headers,
    },
  });

/**
 * Remove a permission from a role.
 */
export const rolesRemovePermission = <ThrowOnError extends boolean = false>(
  options: Options<RolesRemovePermissionData, ThrowOnError>,
) =>
  (options.client ?? client).delete<
    RolesRemovePermissionResponses,
    RolesRemovePermissionErrors,
    ThrowOnError
  >({url: "/api/roles/{id}/permissions/{permission}", ...options});

/**
 * List all users.
 */
export const usersGetAll = <ThrowOnError extends boolean = false>(
  options?: Options<UsersGetAllData, ThrowOnError>,
) =>
  (options?.client ?? client).get<UsersGetAllResponses, UsersGetAllErrors, ThrowOnError>({
    url: "/api/users",
    ...options,
  });

/**
 * Create a new user.
 */
export const usersCreate = <ThrowOnError extends boolean = false>(
  options: Options<UsersCreateData, ThrowOnError>,
) =>
  (options.client ?? client).post<UsersCreateResponses, UsersCreateErrors, ThrowOnError>({
    url: "/api/users",
    ...options,
    headers: {
      "Content-Type": "application/json",
      ...options.headers,
    },
  });

/**
 * Delete a user.
 */
export const usersDelete = <ThrowOnError extends boolean = false>(
  options: Options<UsersDeleteData, ThrowOnError>,
) =>
  (options.client ?? client).delete<UsersDeleteResponses, UsersDeleteErrors, ThrowOnError>({
    url: "/api/users/{id}",
    ...options,
  });

/**
 * Get a user by ID.
 */
export const usersGetById = <ThrowOnError extends boolean = false>(
  options: Options<UsersGetByIdData, ThrowOnError>,
) =>
  (options.client ?? client).get<UsersGetByIdResponses, UsersGetByIdErrors, ThrowOnError>({
    url: "/api/users/{id}",
    ...options,
  });

/**
 * Update a user's profile.
 */
export const usersUpdate = <ThrowOnError extends boolean = false>(
  options: Options<UsersUpdateData, ThrowOnError>,
) =>
  (options.client ?? client).put<UsersUpdateResponses, UsersUpdateErrors, ThrowOnError>({
    url: "/api/users/{id}",
    ...options,
    headers: {
      "Content-Type": "application/json",
      ...options.headers,
    },
  });

/**
 * Get a user's effective permissions (role + direct).
 */
export const usersGetPermissions = <ThrowOnError extends boolean = false>(
  options: Options<UsersGetPermissionsData, ThrowOnError>,
) =>
  (options.client ?? client).get<
    UsersGetPermissionsResponses,
    UsersGetPermissionsErrors,
    ThrowOnError
  >({url: "/api/users/{id}/permissions", ...options});

/**
 * Assign a direct permission to a user.
 */
export const usersAddPermission = <ThrowOnError extends boolean = false>(
  options: Options<UsersAddPermissionData, ThrowOnError>,
) =>
  (options.client ?? client).post<
    UsersAddPermissionResponses,
    UsersAddPermissionErrors,
    ThrowOnError
  >({
    url: "/api/users/{id}/permissions",
    ...options,
    headers: {
      "Content-Type": "application/json",
      ...options.headers,
    },
  });

/**
 * Remove a direct permission from a user.
 */
export const usersRemovePermission = <ThrowOnError extends boolean = false>(
  options: Options<UsersRemovePermissionData, ThrowOnError>,
) =>
  (options.client ?? client).delete<
    UsersRemovePermissionResponses,
    UsersRemovePermissionErrors,
    ThrowOnError
  >({url: "/api/users/{id}/permissions/{permission}", ...options});

/**
 * Get the roles assigned to a user.
 */
export const usersGetRoles = <ThrowOnError extends boolean = false>(
  options: Options<UsersGetRolesData, ThrowOnError>,
) =>
  (options.client ?? client).get<UsersGetRolesResponses, UsersGetRolesErrors, ThrowOnError>({
    url: "/api/users/{id}/roles",
    ...options,
  });

/**
 * Assign a role to a user.
 */
export const usersAddRole = <ThrowOnError extends boolean = false>(
  options: Options<UsersAddRoleData, ThrowOnError>,
) =>
  (options.client ?? client).post<UsersAddRoleResponses, UsersAddRoleErrors, ThrowOnError>({
    url: "/api/users/{id}/roles",
    ...options,
    headers: {
      "Content-Type": "application/json",
      ...options.headers,
    },
  });

/**
 * Remove a role from a user.
 */
export const usersRemoveRole = <ThrowOnError extends boolean = false>(
  options: Options<UsersRemoveRoleData, ThrowOnError>,
) =>
  (options.client ?? client).delete<UsersRemoveRoleResponses, UsersRemoveRoleErrors, ThrowOnError>({
    url: "/api/users/{id}/roles/{roleId}",
    ...options,
  });

export const weatherForecastGet = <ThrowOnError extends boolean = false>(
  options?: Options<WeatherForecastGetData, ThrowOnError>,
) =>
  (options?.client ?? client).get<WeatherForecastGetResponses, unknown, ThrowOnError>({
    url: "/WeatherForecast",
    ...options,
  });
