...
**`AppTelemetry`** — единственный `ActivitySource` приложения, `ProjectWarehouse.Server`. Имя регистрируется через `AddSource`; без регистрации собственные спаны не попадают в экспортёр вовсе.

**`TraceContextCarrier`** — трейс-контекст производителя в проводном виде, парой строк `traceparent`/`tracestate`. Строки, а не `ActivityContext`, потому что запись должна пережить сериализацию, если очередь когда-нибудь станет durable. `Capture()` снимает контекст с `Activity.Current`, `ToLinks()` разворачивает его обратно в `ActivityLink` и отдаёт пустой массив, если контекста не было.

**`TracedMessage<T>`** — конверт «полезная нагрузка + контекст». Очередь принимает голый payload и заворачивает его сама в `EnqueueAsync`, так что вызывающая сторона о телеметрии не знает; потребитель получает `TracedMessage<T>` и открывает спан через `AppTelemetry.StartDetachedQueueConsumer`.
...
Слово «Detached» в имени — не украшение: метод обнуляет `Activity.Current` перед стартом спана, и всё, что идёт дальше по async-потоку, висит уже только на возвращённом спане. В цикле `BackgroundService` ambient-контекст и так пуст, но обнуление делает ссылочную семантику независимой от того, что оставил предыдущий виток. Вызывать метод из потока, которому ambient-контекст ещё нужен, нельзя — связь с родителем будет потеряна молча.

Так покрыта синхронизация маркетплейсов. `MarketplaceSyncQueue` захватывает контекст в `EnqueueAsync` — одинаково для обоих производителей, `MarketplaceSyncScanJob` и `POST /marketplaces/{id}/sync`. `MarketplaceSyncWorker` открывает на каждое сообщение спан `marketplace.sync` с атрибутами `marketplace.account.id` и `marketplace.sync_run.id`; необработанное исключение записывается в спан и переводит его в статус `Error`.

Атрибут `marketplace.sync.scope` вешает на тот же спан `MarketplaceSyncService.RunAsync` — из строки `MarketplaceSyncRun`, а не из сообщения очереди. Ветвление по этапам читает `run.Scope`, и тег обязан показывать то же значение, по которому реально принято решение.
...
Под этим спаном лежат этапы синхронизации, каждый со своим спаном и итоговыми счётчиками:

| Спан | Атрибуты |
|------|----------|
| `marketplace.sync.seller_info` | — |
| `marketplace.sync.warehouses` | `marketplace.warehouses.processed` |
| `marketplace.sync.cards` | `marketplace.cards.processed`, `.created`, `.archived` |
| `marketplace.sync.orders` | `marketplace.orders.created`, `.updated`, `.skipped` |
...
Спан `marketplace.sync` — корневой (см. [Собственные спаны и фоновые очереди](#собственные-спаны-и-фоновые-очереди)), поэтому решение по нему принимается сэмплером независимо от решения по джобе или запросу, который поставил задачу в очередь. При `TraceSampleRatio` меньше единицы одна половина связки может сохраниться без другой; при значении по умолчанию `1.0` сохраняются обе.
...
Единственный писатель — расширение `ILogger.LogFailedResponse` в `Integrations/Ozon/OzonResponseLogging.cs`. Каждый `catch (OzonApiException)` проходит через него, а уровень выбирается по тому, сбой это или ожидаемый ответ площадки:

| Место | Уровень |
|-------|---------|
| `OzonMarketplaceProvider.LogAndWrap` — общая воронка, через неё идут все проброшенные наружу отказы | `Error` |
| `OzonMarketplaceProvider.FetchLabelDocumentAsync` — отказ печати этикеток | `Warning` |
| `OzonMarketplaceProvider.FetchLabelDocumentAsync` — этикетка ещё не готова | `Information` |
| `OzonMarketplaceProvider.ValidateAsync` — площадка отклонила ключ (401/403) | `Warning` |
| `OzonClient.GetPostingStatusAsync` — 404 на забытом Ozon отправлении, глотается на месте | `Warning` |

Печать этикеток понижена до `Warning` не из вкусовщины: `MarketplaceLabelService` отвечает на отказ пачки повторной попыткой по каждому отправлению отдельно, поэтому одна неудачная пачка из N возвращается сюда N+1 раз. На `Error` штатный ретрай выглядел бы как шторм сбоев. По той же причине на `Warning` стоит и отклонённый ключ: неверный Api-Key — это ответ валидации, а не её поломка.

Отсюда деление на два метода в провайдере. `LogAndWrap` — обычный выход: пишет тело и заворачивает `OzonApiException` в `MarketplaceApiException`. `Wrap` только заворачивает и годится для веток, которые пишут тело сами, на своём уровне.
...
Не путать с полем `marketplaceResponse` в `MarketplaceSyncRun.Error`: там лежит **обрезанная до 2000 символов** копия (`OzonMarketplaceProvider.BodyLimit`), и живёт она в Postgres ради текста ошибки в UI. Лимит относится только к ней; к логу — нет.
