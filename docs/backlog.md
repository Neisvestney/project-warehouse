...
экспортёр `otlp` с `sending_queue: {storage: file_storage}` в traces- и logs-пайплайны
`otel/collector.prod.yaml` и вынести адрес туннеля в переменную окружения.

## Метрики в архив телеметрии

**Что это даёт.** Загрузка CPU и GC, размер пула соединений Npgsql, длина очереди
`MarketplaceSyncQueue`, время ответа Ozon Seller API по перцентилям. Инструментация уже установлена:
`AddAspNetCoreInstrumentation` и `AddHttpClientInstrumentation` регистрируют метрики тем же вызовом,
что и трейсы, — нужен только `.WithMetrics(...)` и пайплайн в коллекторе.

**Почему не сделано.** Метрики — непрерывный поток точек: он пишется в файл с одинаковой скоростью
и когда что-то происходит, и когда нет, вытесняя ротацией трейсы, ради которых архив заведён.
Ценность метрик — в запросе по временному окну, а файловый архив запросов не поддерживает: чтобы
увидеть график, придётся выгрузить весь диапазон.

**Триггер.** Появление хранилища, умеющего временные ряды, — либо Prometheus рядом с приложением,
либо локальный SigNoz/Grafana вместо Aspire Dashboard. Тогда добавить `.WithMetrics(...)` с
`AddRuntimeInstrumentation()` и отдельный пайплайн `metrics` с экспортом в это хранилище, минуя
`fileexporter`.
