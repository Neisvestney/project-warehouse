...
досылаются при следующем подключении.

**Почему не сделано.** Ради разбора инцидента постфактум это лишний движущийся узел: очередь на
диске конкурирует за место с самим архивом, а `retry_on_failure` при постоянно закрытом туннеле
держит батчи в памяти коллектора, которому выделено 192 МБ. Пока телеметрию смотрят после события,
а не во время, файлов достаточно.

**Триггер.** Появление класса багов, воспроизводимых только на проде и только вживую — гонка,
исчезающая при повторном прогоне, или зависание, которое нечем поймать по факту. Тогда добавить
экспортёр `otlp` с `sending_queue: {storage: file_storage}` в пайплайны
`otel/collector.prod.yaml` и вынести адрес туннеля в переменную окружения.
...
## Журнал начислений маркетплейса

**Что это даёт.** Начисление (accrual) — одна запись в финансовом учёте площадки о движении денег продавца:
выручка за выкупленный товар, комиссия, логистика и обратная логистика, возврат выручки при возврате товара,
компенсация за потерянный или испорченный возврат и её отзыв, реклама, хранение, штрафы. Ozon отдаёт их
методами `/v1/finance/accrual/*`:

| Метод | Что отдаёт |
|---|---|
| `/v1/finance/accrual/by-day` | Все начисления за день, курсор `last_id` |
| `/v1/finance/accrual/postings` | Начисления по списку отправлений |
| `/v1/finance/accrual/types` | Справочник видов начислений (`type_id`) |

Журнал `MarketplaceAccrual` — ключ `(MarketplaceAccountId, AccrualId)`, дата, категория (`POSTING` / `ITEM` /
`NON_ITEM` / `CONTAINER_FEES`), `TypeId`, номер отправления или услуги (`unit_number`), `Sku`, сумма со знаком и
валюта — закрывает сразу три дыры аналитики:

- **Суммы компенсаций по возвратам.** `/v1/returns/list` сообщает только статус компенсации (см.
  [marketplaces-returns-specification.md](marketplaces-returns-specification.md#компенсации)), деньги приходят
  начислением.
- **Фактическая выплата по заказу** вместо `OrderMarketplaceItem.Payout`, который площадка перестаёт освежать
  после доставки (см. [marketplaces-orders-specification.md](marketplaces-orders-specification.md#финансовые-данные-позиций)).
- **Удержания при возврате** — сколько площадка забрала из выплаты, когда товар вернулся; без этого в режиме
  «Выплата» аналитика не показывает деньги возвратов.

Синк — по дням от отметки на аккаунте с нахлёстом в несколько дней: площадка проводит начисления задним числом,
upsert по `accrual_id` дубли съедает. Какие `type_id` означают компенсацию и декомпенсацию, решается маппингом
справочника в таблице `MarketplaceAccrualType`, а не числами в коде.

**Почему не сделано.** Методы помечены у Ozon как beta, и их контракт может смениться. Прежний источник того же,
`/v3/finance/transaction/list`, отключён. Неизвестно главное для компенсаций: какие `type_id` их обозначают и
приходят ли они с привязкой к отправлению и `Sku` или как `NON_ITEM` без неё. Во втором случае компенсацию
удастся показать только суммой по магазину, без разбивки по товарам. Смешивать это с импортом возвратов, который
от начислений не зависит, значило бы держать готовую часть заложницей непроверенного API.

**Триггер.** Импорт возвратов реализован и работает на проде. Тогда сначала вызвать `/v1/finance/accrual/types`
и `/v1/finance/accrual/postings` по нескольким отправлениям с `CompensationStatus = Received` на живом магазине,
ответить на вопросы выше и только после этого проектировать модель.
