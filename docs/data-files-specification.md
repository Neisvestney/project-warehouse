...
Файл, привязанный 1:1, может получить собственный маршрут у владельца, если потребителю известен только
владелец: `GET /api/users/{id}/avatar?width=128` отдаёт фото сотрудника по идентификатору пользователя, не
заставляя фронтенд сначала вычитывать `UserDetailDto`, чтобы узнать `Guid` файла. Это тонкая обёртка — сам
поток строит общий сервис отдачи (см. [«Отдача»](#отдача)), а отсутствие файла и несуществующий владелец
дают один и тот же `404 DataFileNotFound`.
...
Превращение строки `DataFile` в поток — обязанность `IDataFileContentService` (`OpenOriginalAsync` /
`OpenPreviewAsync`), а не контроллера: формат превью, дисковый кэш и «не апскейлить» заданы один раз для всех
эндпоинтов, которые отдают содержимое. Сервис возвращает `DataFileContent` (поток + content type + имя +
`ETag`) или `null`, когда байтов в хранилище нет, и бросает `DataFileNotAnImageException`, если превью
запрошено у недекодируемого файла. Заголовки и `Content-Disposition` навешивает `AppControllerBase.StreamDataFile`:
