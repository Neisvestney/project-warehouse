...
Заказы, ушедшие из `awaiting_deliver`, **из выдачи `unfulfilled/list` исчезают**, и отличить «отгружено» от «отменено» по факту исчезновения невозможно. Поэтому вторым шагом прогона идёт сверка: для всех `MarketplaceOrder` аккаунта, чей `Status` ещё не финальный (`Delivered` / `Cancelled`), запрашивается текущее состояние на площадке. Обновляются `Status`, `RawStatus`, `RawSubstatus`, `TrackingNumber`, `StatusSyncedAt`.

Спрашивается площадка **пачками** через `POST /v4/posting/fbs/list` с фильтром `order_numbers` — сотня номеров за вызов (предел Ozon и по `order_numbers`, и по `limit`), с курсорной пагинацией внутри пачки. Четырёх полей, из которых складывается `ExternalPostingStatus`, в ответе списка ровно столько же, сколько в карточке отдельного отправления, так что поштучный запрос здесь ничего не добавлял бы.
