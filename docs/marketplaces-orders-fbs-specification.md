...
Источник — `financial_data` в ответах обоих posting-эндпоинтов, включаемый флагом `with.financial_data`: в `unfulfilled/list` при обнаружении и в `/v4/posting/fbs/list` при догоне статусов. Отдельного запроса это не стоит: блок приезжает в тех же вызовах, которыми отправления обнаруживаются и сверяются. Позиции связываются с финансовыми строками по `product_id`, который в `products[]` называется `sku`.
...
**Импорт** отправлений запускается только вручную — кнопкой «Синхронизировать заказы» на `/operations/orders/fbs` (`Scope = Orders`). **Догон статусов** ходит и сам: он вынесен в `Scope = OrdersBackground`, входит в `All` и потому приезжает с каждым фоновым прогоном `MarketplaceSyncScanJob` по интервалу аккаунта.

Граница проходит ровно по тому, создаёт ли шаг заказы. Импорт при неразобранном каталоге порождает гору пропусков, а пропуск, который никто не видит в момент появления, бесполезен — поэтому за импортом должен стоять человек, нажавший кнопку. Догон статусов не создаёт ничего: он освежает то, что уже импортировано, и пропускать ему нечего.

`Scope = Orders` выполняет обе фазы: импорт, затем догон. Обратное неверно — `OrdersBackground` импорт не запускает.
...
- **Фоновый импорт заказов** — новые отправления тянутся только вручную из модалки (статусы уже импортированных освежаются фоном). Автоматический импорт по расписанию потребует политики для пропущенных заказов, которую сейчас отрабатывает человек, глядя в итоги прогона.
