...
Источник — `financial_data` в ответе `unfulfilled/list`, включаемый флагом `with.financial_data`. Отдельного запроса это не стоит: блок приезжает в том же вызове, которым отправления и обнаруживаются. Позиции связываются с финансовыми строками по `product_id`, который в `products[]` называется `sku`.
...
**Деньги — снимок на момент импорта и не обновляются никогда.** Отправление попадает в WMS уже в `awaiting_deliver`, то есть после того, как покупатель заплатил, — пересчитывать нечего. Фактические начисления после доставки живут в `/v3/finance/transaction/list` и в модуле не используются.
...
| Источник финансовых данных | **`with.financial_data` в `unfulfilled/list`** | Ноль дополнительных запросов; `customer_price` есть только там, финансовые методы его не отдают |
| Обновление финансовых данных | **Снимок при импорте, без обновления** | В `awaiting_deliver` покупатель уже заплатил; факт после доставки — область `/v3/finance/transaction/list` |
...
- **Живучесть `customer_price` в FBS.** Из ответа `/v2/posting/fbo/list` поле убрано; в спецификации `/v4/posting/fbs/unfulfilled/list` оно остаётся. Проверяется тем, приходит ли оно заполненным на живом отправлении: все финансовые поля необязательны, и пустое значение модуль переживает.
