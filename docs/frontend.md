...
| Файл | Что держит |
|------|------------|
| `batchGroups.ts` | группы массовой сборки: `SelectedTaskInfo`/`BatchTarget`/`BatchGroup`, сборка групп из выбранных заданий и `withoutExcludedTasks` — группа без убранных заданий |
| `GroupTasksDialog.tsx` | список заданий группы с поиском по номеру заказа и отправления: откуда задание убирают из этой позиции |
...
группы. Строка раскрывается по клику в любое место, кроме кнопки в правой колонке, и показывает панель
...
два одинаковых состава. Выбор — и ячейки простой группы, и всего остального — живёт в раскрытой панели;
столбец «Источник» только называет ячейку («нужна ячейка», пока её нет). На телефоне ячейку простой группы
по-прежнему выбирают прямо в карточке — раскрывать там нечего.

Дефолтную ячейку склада в `GroupState` кладёт `useGroupDefaultNode`, а не сам пикер: пикер смонтирован только
у раскрытой группы, а ячейка, которой человек не касался, всё равно должна уехать в запрос. Хук висит на
строке и на карточке — там, где группа живёт всегда, — и на группе, открытой полноэкранным экраном состава,
где карточек уже нет.
...
**Группу можно убрать из сборки.** Крестик в правой колонке строки (на карточке — рядом со статусным чипом)
переводит группу в исключённые: строка гаснет, вместо источника и статуса — прочерк и чип «убрана». На месте
крестика встаёт стрелка отмены, она же возвращает группу обратно. `GroupState` исключённой группы сохраняется, поэтому возврат поднимает
заданный состав целиком — убрать по ошибке не значит потерять работу. Исключённые группы выпадают из всего
остального: из подсчёта неготовых, из `items` запроса и из числа в кнопке отправки, которое считает задания,
покрытые оставшимися группами, а не все выбранные. Когда убраны все — кнопка гаснет с подписью «Все группы
убраны из сборки».

**Отдельные задания убираются внутри группы.** Раскрытая строка (и экран состава на телефоне) начинается со
сводки «Позиций: N · исключено M · осталось K» и кнопки «Задания (x из y)», которая открывает
`GroupTasksDialog` — список заданий этой группы с локальным поиском по номеру заказа и номеру отправления.
Строки списка исключаются тем же крестиком и возвращаются той же стрелкой, что и группы целиком. Исключение
по-групповое: одно и то же задание может уйти из одной позиции и остаться в другой.

Механика живёт в `batchGroups.ts`: `withoutExcludedTasks` отдаёт группу без исключённых `targets` и с
пересчитанным `totalNeeded`, и вниз по дереву идёт уже она. Поэтому за сводкой сами собой едут проверка
остатка, число копий состава (`needTimes`), «Итого» в строке — там, где часть заданий убрана, показывается
`K из N` — и тело запроса. Группа, из которой убрали все задания, получает чип «нет заданий» и выпадает из
отправки, но остаётся в таблице: вернуть задания можно только через её же диалог. Состав у такой группы не
показывается — вместо него строчка «Все задания этой позиции убраны из сборки», потому что задавать его
некуда.

**Закрыть можно только кнопкой «Отмена» или `Esc`, и оба пути спрашивают подтверждение.** `onClose` диалога
пропускает только `escapeKeyDown`, так что клик по подложке ничего не делает, а `useBackClosable` поднят с
`{blockBack: true}` — «назад» возвращает запись обратно в стек вместо закрытия. Подтверждение (`showConfirm`) и
запрет закрытия во время отправки живут в `BatchAssemblyContent`, который владеет мутацией; внешний `Dialog`
достаёт `requestClose` через `useImperativeHandle`. Собранный состав группы живёт в `GroupState` и нигде
больше: случайный промах мимо диалога стирал бы разом всю ручную работу по всем группам.
