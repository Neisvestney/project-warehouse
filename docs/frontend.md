...
| `fulfillmentStatus.ts` | `SlotStatus` (`filled`/`total`), из которого растут все «N из M» |
...
массовой сборке. В одиночном диалоге известная нехватка держит слот незаполненным — сервер всё равно откажет; в
массовой сборке она показывается чипом `остаток N — на M не хватит`, но отправку не блокирует: там частичный
отказ уже разложен по строкам групп.
...
группы. Строка раскрывается по клику в любое место, кроме зоны выбора ячейки, и показывает адресатов группы —
заказ, коробку и количество, первые пять с кнопкой «Ещё N», потому что группа может собрать десятки заданий, — и
панель состава, который задаётся один раз и копируется на каждое задание. Копий ровно столько, сколько штук в
