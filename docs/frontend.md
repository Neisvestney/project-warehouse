...
**Закрыть можно только кнопкой «Отмена».** У диалога нет `onClose`, поэтому клик по подложке и `Esc` ничего не
делают, а `useBackClosable` поднят с `{blockBack: true}` — «назад» возвращает запись обратно в стек вместо
закрытия. Собранный состав группы живёт в `GroupState` и нигде больше: случайный промах мимо диалога стирал бы
разом всю ручную работу по всем группам.
