...
- **Read endpoints that POST**: three reads take a body. `POST /api/statistics/stock-movements/pivot` — its
  ...
  body rather than being split across body and query. `POST /api/statistics/stock-movements/cell` carries one
  such metric for the same reason. `POST /api/catalog/for-select/by-ids` — a selection
